  ...
  chatIndexRef.child(me.uid).on("value", (snap) => {
    ...
    if (notifyReady) {
      Object.keys(next).forEach((peer) => {
        ...
        if (isNew && !chatOpen) {
          notify(displayName(peer), previewOf({ text: after.last }));
        }
      });
    }
...
function renderChatList() {
  ...
  if (!chatFilterText || "group chat".includes(chatFilterText)) {
    const groupMeta = chatIndex[GROUP_ID] || {};
    fragment.appendChild(
      buildChatRow({
        id: GROUP_ID,
        isGroup: true,
        name: "Group Chat",
        sub: groupMeta.last ? groupMeta.last : Object.keys(usersMap).length + " members",
        ts: groupMeta.ts,
        unread: 0
      })
    );
...
function openChat(target) {
  ...
  /* Receipt tracking is per conversation */
  newestIncoming = 0;
  markedSeenTs = 0;
  markedDeliveredTs = 0;
  ...
  const onAdd = query.on("child_added", (snap) => {
    ...
    if (!isMine(data)) {
      ...
      if (activeChat === GROUP_ID && notifyReady && document.hidden) {
        notify(data.sender || "New message", previewOf(data));
      }
    }
  });
...
function markSeen() {
  ...
  const now = Date.now();
  if (now - lastSeenWrite < APP_CONFIG.RECEIPT_THROTTLE) return;
  lastSeenWrite = now;
...
function markDelivered() {
  ...
  const now = Date.now();
  if (now - lastDeliveredWrite < APP_CONFIG.RECEIPT_THROTTLE) return;
...
function pushMessage(payload) {
  ...
  return write(ref.set(data), "Message failed to send.").then((ok) => {
    if (ok && activePeer) updateChatIndex(preview);
    return ok;
  });
}
...
function notify(title, body) {
  ...
  try {
    const n = new Notification("TINDER · " + title, { body: body, tag: "tinder-msg" });
...
/* Image viewer */
$("#imageViewer").onclick = () => $("#imageViewer").classList.add("hidden");
...
document.addEventListener("keydown", (e) => {
  ...
  if (!$("#imageViewer").classList.contains("hidden")) return $("#imageViewer").classList.add("hidden");
...
["settingsModal", "membersModal"].forEach((id) => {
